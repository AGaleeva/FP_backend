...
public class TranslatorsService {
    ...
    public TranslatorsService(TranslatorsRepository translatorsRepository, CityRepository cityRepository) {
    ...
    public TranslatorsInfo save(TranslatorsInfo translators, String cityName) {
        try {
            ...
            return translatorsRepository.save(translators);
        } catch (Exception e) {
    ...
    public List<TranslatorsInfo> findByTitle(String title, String cityName) {
        List<TranslatorsInfo> list = findAll(cityName).stream().filter((r) -> r.getTitle().equalsIgnoreCase(title.trim())).toList();
        if (list.isEmpty()) {
            throw new TranslatorNotFoundException(String.format(
