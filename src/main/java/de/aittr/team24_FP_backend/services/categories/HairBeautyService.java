...
public class HairBeautyService {
    ...
    public HairBeautyService(HairBeautyRepository hairBeautyRepository, CityRepository cityRepository) {
    ...
    public HairBeautyInfo save(HairBeautyInfo hairBeauty, String cityName) {
        try {
            ...
            return hairBeautyRepository.save(hairBeauty);
        } catch (Exception e) {
    ...
    public List<HairBeautyInfo> findByTitle(String title, String cityName) {
        List<HairBeautyInfo> list = findAll(cityName).stream().filter((r) -> r.getTitle().equalsIgnoreCase(title.trim())).toList();
        if (list.isEmpty()) {
            throw new HairBeautyNotFoundException(String.format(
