...
public class ChildrenService {
    ...
    public ChildrenService(ChildrenRepository childrenRepository, CityRepository cityRepository) {
    ...
    public ChildrenInfo save(ChildrenInfo childrenInfo, String cityName) {
        try {
            ...
            return childrenRepository.save(childrenInfo);
        } catch (Exception e) {
    ...
    public List<ChildrenInfo> findByTitle(String title, String cityName) {
        List<ChildrenInfo> list = findAll(cityName).stream().filter((r) -> r.getTitle().equalsIgnoreCase(title.trim())).toList();
        if (list.isEmpty()) {
            throw new ChildrenNotFoundException(String.format(
